...
**第九轮：RL 训练用量实证（2026-09-13，用户问题「RL 数据用多少/全量吗」）：** 真实运行的任务池×步数：**DeepSWE=4,500 任务×200 步**（64 prompts×8 rollouts/迭代=512 容器，200 步即 +20% SV，64×H100×6 天）；**LEGO-RL=2,699 任务×3 epochs/126 步**（G=8，任务循环复用）；OT-Agent≈700 任务；NeMo-RL SWE2=8 prompts/步×G=8；KAT-V2.5 未披露 RL 规模（只披露环境侧：AutoBuilder 10 万+环境、留用任务 10 万+、构建成功率 16.5%→57.2%）；MiniMax/GLM-5 为万级任务（大厂档）。结论：①**无人全量训**——池子都是筛后的 700-4,500（LEGO 仅占候选 7.3%）；②**步数小而增益快**（126-200 步档），任务靠循环采样复用而非一次性消费；③总 rollout 量级 ~10 万条（DeepSWE 200×512）。对我们的定标：12 任务够 smoke；正式首轮 50-100 带内任务×G=8×几十步≈4 千-2 万 rollout（业界量的 1-10%，与 2-4 卡对 64-192 卡的算力比一致——差异化本就是小算力+OPD 样本效率）。附带收获：KAT 的 hint-boosted recovery（过程级提示把零解任务救活到 ~20% 通过率）可作我们 69 个零解任务的后续复活手段。

**第十轮：扩池通路裁决（2026-09-13，用户指令做 smith 环境准备后核实）：** **SWE-smith 与本地环境路线不兼容**——原始 schema 仅 instance_id/patch/F2P/P2P/image_name/repo/problem_statement：环境=预构建 Docker 镜像（无 base_commit、无 test_patch），无法走本地 venv+源码 tarball+冻结评测器路线；平台 Docker 层缺口是已知问题 → **扩池改走 gym 池扩仓库**：剩余 2,066 eligible 分布=pandas 722/MONAI 355/moto 333/mypy 255/dvc 221/modin 107/conan 73（全部管线兼容）。prepare_rl_tasks.py 增加 --repos 参数（覆盖原 4 仓库硬编码）；探产批 300 候选（pandas+mypy+moto，冻结顺序前缀）已启动（CPU，nohup，logs/prepare-rl-round1b.log；产物 round1b 独立目录不覆写 round1）。产出率账：按 gym 实测 qual 30%×带内 13.8%，50-100 带内任务需 ~1200-2400 候选全流程；探产批先实测新仓库的 qual/带内率再定全量（新仓库分布可能不同）。KAT hint-boosted recovery（零解任务提示救活到 ~20%）列为零解池复活备选。

**第十一轮：扩池探针实证（2026-09-13）：** 任务源产出率对比——原 curated 4 仓库 qual 率 30%（88/291）；新仓库探针 **mypy 75%（9/12）、moto ≈83%（5/6，唯一失败为 120s 测试超时的合法淘汰）**；正式批 588 候选在飞维持 77.6%。含义：任务源选择对池子效率的影响大于后续任何环节——mypy/moto 两个仓库的每候选产出是 curated 池的 2.5 倍+。moto/mypy 的难度筛选带内率待筛（可能同样高于 curated 的 13.8%）。配方工程教训入 PROGRESS 变更日志（4 处基建 bug+7 轮配方迭代）。

**第十二轮：奖励栈 slime_dsh 实现落地（2026-09-13，锁定设计 §3 执行）：** 按已锁定的 reward-design-stage2-grpo.md 实现计划，四个模块全部 CPU 完成带单测（scripts/check_reward_stack.py 11/11 PASS，`.venv-train-rl` CPU + 真实 Qwen3.5 tokenizer）：① `slime_dsh/reward.py`——二值核（金标 grading_solved）+未完成惩罚开关（`DSH_C_UNFINISHED`，默认 0=纯二值起步）+失败原因四分类枚举（environment/model/budget/none，GLM-5 "record the failure reason" 字段化）；② `slime_dsh/group_repair.py`——GLM-5 组剔除+填充/丢弃（挂 `--rollout-all-samples-process-path`；环境崩溃槽位轮转重复有效槽填充、有效≤半数整组丢弃）+独立组归一化钩子（挂 `--custom-reward-post-process-path`，按 instance_id 组、统计剔除环境崩溃样本）；③ `slime_dsh/format_penalty.py`——Qwen3CN 规则级工具调用校验（`<tool_call>` JSON 块：JSON 可解析/name/arguments dict/可选工具名白名单/标签配对）+**字节级精确 token 定位**（字节级 BPE 的 convert_ids_to_tokens+GPT-2 bytes-to-unicode 逆映射，逐 token 字节串拼接与解码文本 utf-8 字节全等断言，真 tokenizer 验证）+custom advantage 钩子（复刻 grpo/gspo/cispo 基线后在监督 token 上减 `DSH_C_FMT`，与 loss_mask 正交——观察区回显的工具调用文本不受罚）；④ `slime_dsh/blocker.py`——Qwen3CN 防作弊拦截器（仓库链接∧网络关键字→阻断），实现在 adapter 回复层把违规命令改写为无害 echo、反馈作为工具结果回注（轨迹可继续而非终止），wire/manager 消息同源改写保证下轮 echo 匹配不 fork，`DSH_BLOCKER=1` 显式启用（默认关，保持既有证据路径逐字节不变）。**实现核实的两个 slime 事实**（对讲解有用）：(a) `remove_sample` 只清 loss mask 不改优势归一化参与（arguments.py 帮助文本明示）——组修复缺口确证；(b) `_post_process_rewards` 的 reshape 依赖样本总数==K×batch，多分段 agent 轨迹恒不满足→退化为 `view(-1, N)` 的**全批全局均值归一化**——组归一化钩子存在的第二个理由。**顺带发现（先存问题，非本轮引入，git 对照已验证）**：check_slime_harness.py 的 3 个 lifecycle 用例自本地后端绑定（09-11/12）后即失效——其 fixture monkeypatch 上游 `E2BSandbox`，而 `boot_agent_sandbox` 已被 `local_backend.bind` 整体替换，`fake-image` 直接进 `boot_local_sandbox` 报错；4 个纯契约用例+dsh-check+local-backend 检查仍全过。修复需把 fixture 改造为 local 契约（image='local'+注册 fixture 案例），列队待办。**超参边界**：所有 c_*/开关经环境变量传输、默认全零/关——数值本身属 GRPO smoke 超参单，待用户呈批；格式罚启用时需同时关 `--normalize-advantages`（后置白化会把罚项重标度掉），已写入模块 docstring。

**第十三轮：round1b 扩池批收尾+三份 registry 合并（2026-09-14）：** 588 候选全量处理完成——原批 [0..197] 198 任务（150 OK/47 infrastructure_startup_failure/1 其他，11:41 UTC 被停未写 registry）、w1 [197..397) 200 任务 151 OK（326.4 min，17:09 UTC 收尾）、w2 [397..588) 191 任务 154 OK（287.3 min，16:30 UTC 收尾）。**合并要点与两处脚本事实**：①prepare_rl_tasks.py 的 registry/parquet/consumed 只在批末统一落盘（脚本 143-149 行）——原批被停于中途，其 150 个合格任务的 registry 从未写出；但逐任务产物都在盘上（rollout case 目录+每 case 的 runtime-config.json 含 `task_python`，与 w1 registry 151 条逐一比对零不一致），故从盘重建而非重跑（重跑需 ~2.6h CPU，重建秒级）。②原批被停前多处理了 1 个任务（[198/588]=index 197=w1 的 `--skip 197` 首任务 `python__mypy-15846`，两边都 qual=OK）——合并脚本原版对重叠一票否决，修正为：python 路径一致性硬校验+worker registry 优先去重+重叠记入报告。合并脚本顺带修复 `gp` 仅在局部作用域导入导致 main 内 NameError 的问题，并新增全量 454 条 case_dir/python/workdir 存在性校验。**结果**：454 个合格任务（getmoto/moto 256 + python/mypy 198，总合格率 454/588=77.2%，与探针预估 75-83% 吻合），产物 configs/agent-rl/local-task-registry-r1b-merged.json + data/agent-rl/rl-round1b-merged-prompts.parquet（454 行，metadata 契约与 local_backend 一致；重叠任务与 w1 parquet 逐字段相等验证）+ runtime/agent-rl/round1b-merge-report.json（consistency=true）。**池子现状**：待难度筛选的是 round1b 新增 454（curated 87 已于 09-12/13 筛过：带内 12/全零 69/全解 4，结果沿用不重筛——D15 当时裁决就是扩池而非放宽预算重筛零解任务）；454×4=1,816 attempts，1×H100 约 3 天，等用户安排 GPU；合并脚本 scripts/merge_round1b_registries.py。
（2026-09-14 口径修正：本条与 PROGRESS 初版误写「87+454=541 待筛」，把已筛过的 87 也计入——用户问询后修正为 454 待筛、87 结果沿用。）
